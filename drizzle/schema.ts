...
export const imageDetailsForAiGeneration = pgTable("image_details_for_ai_generation", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	characterId: uuid("character_id").notNull(),
	...
	background: text(),
	atmosphere: text(),
	magicalElements: text("magical_elements"),
	culturalElements: text("cultural_elements"),
	temporalContext: text("temporal_context"),
	specialEffects: text("special_effects"),
	artisticInfluences: text("artistic_influences"),
	detailLevel: varchar("detail_level", { length: 50 }).default('high'),
	generationPrompt: text("generation_prompt"),
	negativePrompt: text("negative_prompt"),
	aspectRatio: varchar("aspect_ratio", { length: 20 }).default('1:1'),
	scale: varchar({ length: 50 }).default('5'),
	qualityLevel: varchar("quality_level", { length: 50 }).default('hd'),
	...
	foreignKey({
		columns: [table.characterId],
		foreignColumns: [characters.id],
		name: "image_details_character_id_characters_id_fk"
	}).onDelete("cascade"),
	unique("image_details_character_id_unique").on(table.characterId),
]);
