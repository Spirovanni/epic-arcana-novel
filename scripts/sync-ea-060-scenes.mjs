/**
 * Sync EA-060 Scene Updates to Neon DB
 * 
 * This script reads EA-060 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (60) → DB chapter_number (60) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 20") → Book 2's 20th chapter
 * 
 * Usage: node scripts/sync-ea-060-scenes.mjs
 */
...
async function syncEA060Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-060 scenes to database\n');

        // Find EA-060 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-060');

        if (!eaData) {
            throw new Error('EA-060 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-060: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-060');
        }
