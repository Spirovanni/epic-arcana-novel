...
async function verifyEA219() {
    console.log('🔍 Verifying EA-219 Import...\n');

    const chapterNumber = 219;
    ...
    try {
        ...
        if (!chapter) {
            console.error('❌ Chapter 219 not found');
            return;
        }
        ...
        // Expected scene count (4 for EA-219)
        ...
        for (const scene of chapterScenes) {
            ...
            console.log(`   🎯 Focus: ${scene.focus || 'not set'}`);
            console.log(`   📋 Preliminary Scene Focus: ${scene.preliminarySceneFocus || 'not set'}`);
            console.log(`   📝 Preliminary Scene Description: ${scene.preliminarySceneDescription || 'not set'}`);
        ...
        console.log('\n' + '='.repeat(50));
        console.log('✅ Verification complete for EA-219');
