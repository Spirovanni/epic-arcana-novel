...
function createSimplifiedScenes(chapter: Book2Chapter, chapterNumber: number): ChapterUpdate {
  ...
  // Determine core emotion based on chapter content
  const getCoreEmotion = (title: string, summary: string): string => {
  ...
  const getSceneTone = (title: string, summary: string): string => {
    const content = (title + ' ' + summary).toLowerCase();
  ...
  return {
    ...
    tense: "Past Tense",
    core_emotion: getCoreEmotion(chapter.title, chapter.summary),
    scene_tone: getSceneTone(chapter.title, chapter.summary)
  };
}
...
async function updateBook2Scenes() {
  try {
    ...
    const chapterMap = new Map<number, Book2Chapter>();
        ...
        if (obj.hasOwnProperty(key)) {
          ...
          if (typeof value === 'object' && value !== null) {
            ...
            if (value.unique_identifier && typeof value.unique_identifier === 'string') {
              const match = value.unique_identifier.match(/^STG 2\.(\d+)\.(\d+)\.(\d+)$/);
              if (match) {
                const chapterNum = parseInt(match[3]);
                const chapterData = chapterMap.get(chapterNum);
                
                if (chapterData) {
                  console.log(`Updating chapter ${chapterNum}: ${chapterData.title}`);
                  
                  // Create simplified scenes structure
                  const chapterUpdate = createSimplifiedScenes(chapterData, chapterNum);
                  
                  // Add scenes to the chapter
                  value.scenes = chapterUpdate.scenes;
                  value.pov = chapterUpdate.pov;
                  value.tense = chapterUpdate.tense;
                  value.core_emotion = chapterUpdate.core_emotion;
                  value.scene_tone = chapterUpdate.scene_tone;
