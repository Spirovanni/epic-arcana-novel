...
async function fixEA004to006WithBeatGoal() {
    console.log('🔧 Fixing chapter_scene_focus for EA-004, EA-005, EA-006\n');
    console.log('   Using beat_goal field\n');
    ...
    try {
      ...
      `);
            ...
            for (const scene of scenes) {
                ...
                const beatGoal = scene.beat_goal || '';

                if (beatGoal) {
                    // Format as "ChXSY: [beat_goal]"
                    const chapterSceneFocus = `Ch${chapterNum}S${sceneNum}: ${beatGoal}`;
          ...
          `);
                    ...
                    console.log(`    → "${chapterSceneFocus.substring(0, 70)}..."`);
                    totalUpdated++;
                } else {
                    console.log(`  ⚠️  Scene ${sceneNum}: No beat_goal`);
                }
            }
        }
