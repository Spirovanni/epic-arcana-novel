/**
 * Sync EA-050 Scene Updates to Neon DB
 * 
 * This script reads EA-050 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * Usage: node scripts/sync-ea-050-scenes.mjs
 */
...
async function syncEA050Scenes() {
  try {
    ...
    console.log('\n🎯 Syncing EA-050 scenes to database\n');
    
    // Find EA-050 in the outline structure
    ...
    const ea050Data = findChapterById(outlineData, 'EA-050');
    
    if (!ea050Data) {
      throw new Error('EA-050 not found in data/l_outline.json');
    }
    
    console.log(`📚 Found EA-050: ${ea050Data.specific_task_group_title || ea050Data.title}`);
    console.log(`   Chapter Number: ${ea050Data.all_chapter}`);
    console.log(`   Novel Book: ${ea050Data.novel_book}`);
    console.log(`   Unique ID: ${ea050Data.unique_identifier}`);
    console.log(`   Scenes: ${ea050Data.scenes?.length || 0}`);
    
    if (!ea050Data.scenes || ea050Data.scenes.length === 0) {
      throw new Error('No scenes found in EA-050');
    }
    ...
    let chapterQuery = await client.query(
      `SELECT id, chapter_number, title, unique_identifier 
       ...
       LIMIT 1`,
      [ea050Data.unique_identifier]
    );
    
    if (chapterQuery.rows.length === 0) {
      console.log(`   Trying to find by chapter_number: ${ea050Data.all_chapter}`);
      chapterQuery = await client.query(
        `SELECT id, chapter_number, title, unique_identifier 
         FROM chapters 
         WHERE chapter_number = $1 
         LIMIT 1`,
        [ea050Data.all_chapter]
      );
    }
    
    if (chapterQuery.rows.length === 0) {
      throw new Error(
        `Chapter not found in database. Please create chapter ${ea050Data.all_chapter} ` +
        `(unique_id: ${ea050Data.unique_identifier}) before syncing scenes.`
      );
    }
    ...
    console.log(`\n📝 Processing ${ea050Data.scenes.length} scenes...\n`);
    
    for (const scene of ea050Data.scenes) {
    ...
    console.log(`🎯 Chapter: ${chapter.chapter_number} - ${ea050Data.specific_task_group_title}`);
    console.log('='.repeat(60));
