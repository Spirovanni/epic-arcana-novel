...
/**
 * Add EA-057 chapter from l_outline.json to Neon DB
 * Book 2, Chapter 17 (all_chapter 57)
 */
...
function findEa057(data: any): any {
...
async function addEa057() {
  console.log('🔍 Finding EA-057 in l_outline.json...\n');
  ...
  try {
    ...
    const chapterData = findEa057(outlineData);
    
    if (!chapterData) {
      console.log('❌ EA-057 not found in l_outline.json');
      process.exit(1);
    }
    
    console.log('✅ Found EA-057:');
    ...
    const chapterDbData = {
      chapterId: chapterData.id, // EA-057
      uniqueIdentifier: chapterData.unique_identifier.startsWith('EA-') ? null : chapterData.unique_identifier, // STG 2.2.2.1
    ...
    console.log('\n📊 Chapter EA-057 successfully added/updated in Neon DB!');
    
  } catch (error) {
    console.error('\n❌ Error adding EA-057:', error);
    process.exit(1);
  }
}

addEa057().catch(console.error);
