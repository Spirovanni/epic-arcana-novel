/**
 * Sync EA-068 Scene Updates to Neon DB
 * 
 * This script reads EA-068 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (68) → DB chapter_number (68) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 28") → Book 2's 28th chapter
 * 
 * Usage: node scripts/sync-ea-068-scenes.mjs
 */
...
async function syncEA068Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-068 scenes to database\n');

        // Find EA-068 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-068');

        if (!eaData) {
            throw new Error('EA-068 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-068: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-068');
        }
