...
async function verifyEA220() {
    console.log('🔍 Verifying EA-220 Import...\n');

    const chapterNumber = 220;
    ...
    try {
        ...
        if (!chapter) {
            console.error('❌ Chapter 220 not found');
            return;
        }
        ...
        // Expected scene count (4 for EA-220)
        ...
        for (const scene of chapterScenes) {
            ...
            console.log(`   🎯 Focus: ${scene.focus ? scene.focus.substring(0, 100) + '...' : 'not set'}`);
            console.log(`   📋 Preliminary Scene Focus: ${scene.preliminarySceneFocus || 'not set'}`);
            console.log(`   📝 Preliminary Scene Description: ${scene.preliminarySceneDescription ? scene.preliminarySceneDescription.substring(0, 100) + '...' : 'not set'}`);
            console.log(`   🎯 Chapter Scene Focus: ${scene.chapterSceneFocus || 'not set'}`);
            console.log('');
        }
        ...
        console.log('\n' + '='.repeat(50));
        console.log('✅ Verification complete for EA-220');
