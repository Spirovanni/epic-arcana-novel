...
const BOOK2_JSON_PATH = path.join(process.cwd(), 'lore', 'book2_expanded_codex.json');
...
async function seedBook2Data() {
  ...
  console.log('Loading book2 expanded codex data...');
  const book2Data = await loadJSON(BOOK2_JSON_PATH);

  console.log('Starting book2 expanded codex seeding...');
  ...
  // Check if book2 already exists
  const existingBook = await db.select().from(schema.books).where(eq(schema.books.bookNumber, 2));
  let bookId;

  if (existingBook.length === 0) {
    console.log('Creating new book2 record...');
    const newBook = await db.insert(schema.books).values({
      seriesId: seriesId,
      bookNumber: 2,
      uniqueIdentifier: book2Data.book_unique_identifier,
      title: book2Data.fantasy_book_name,
      subject: book2Data.book_subject,
      focus: book2Data.focus_of_book,
      description: book2Data.book_description,
      themes: [book2Data.unique_theme],
      enneagramType: book2Data.enneagram,
      militaryComponent: book2Data.military,
      businessModel: book2Data.business_model_generation,
      personalityType: book2Data.personality_type,
      coveryCoveyHabit: book2Data['9_habits_covey'],
      associatedSin: book2Data.sin,
    }).returning({ id: schema.books.id });
    bookId = newBook[0].id;
  } else {
    bookId = existingBook[0].id;
    console.log(`Using existing book2 ID: ${bookId}`);
  ...
  for (const section of book2Data.sections) {
  ...
  console.log(`Seeding completed! Created ${chapterCount} chapters for book2.`);
  await pool.end();
}
...
async function main() {
  try {
    await seedBook2Data();
    console.log('Book2 expanded codex seeding completed successfully!');
  } catch (error) {
