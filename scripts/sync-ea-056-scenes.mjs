/**
 * Sync EA-056 Scene Updates to Neon DB
 * 
 * This script reads EA-056 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (56) → DB chapter_number (56) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 16") → Book 2's 16th chapter
 * 
 * Usage: node scripts/sync-ea-056-scenes.mjs
 */
...
async function syncEA056Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-056 scenes to database\n');

        // Find EA-056 in the outline structure
        ...
        const ea056Data = findChapterById(outlineData, 'EA-056');

        if (!ea056Data) {
            throw new Error('EA-056 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-056: ${ea056Data.specific_task_group_title || ea056Data.title}`);
        console.log(`   all_chapter (absolute): ${ea056Data.all_chapter}`);
        console.log(`   novel_book: ${ea056Data.novel_book}`);
        console.log(`   chapter (book-relative): ${ea056Data.chapter}`);
        console.log(`   unique_identifier: ${ea056Data.unique_identifier}`);
        console.log(`   Scenes: ${ea056Data.scenes?.length || 0}`);

        if (!ea056Data.scenes || ea056Data.scenes.length === 0) {
            throw new Error('No scenes found in EA-056');
        }
       ...
       LIMIT 1`,
            [ea056Data.all_chapter]
        );

        let chapterId = null;

        if (chapterQuery.rows.length === 0) {
            console.log(`   Chapter ${ea056Data.all_chapter} not found. Creating it...`);
         ...
         RETURNING id`,
                [
                    book2Id,
                    ea056Data.all_chapter,
                    ea056Data.unique_identifier,
                    ea056Data.specific_task_group_title,
                    ea056Data.focus_area,
                    ea056Data.specific_task_group_description,
                    ea056Data.specific_task_group_tagline
                ]
            );
            ...
            if (chapter.title !== ea056Data.specific_task_group_title) {
                console.log(`   Updating chapter title: ${ea056Data.specific_task_group_title}`);
                await client.query(
                    `UPDATE chapters SET 
               title = $1, 
               unique_identifier = $2,
        ...
        console.log(`\n📝 Processing ${ea056Data.scenes.length} scenes...\n`);

        for (const scene of ea056Data.scenes) {
            ...
            const sceneData = {
                ...
                description: scene.description || scene.setup || null, // Fill description with setup if description missing
        ...
        console.log(`🎯 Chapter: ${ea056Data.all_chapter} (Book 2, Ch 16) - ${ea056Data.specific_task_group_title}`);
        console.log('='.repeat(70));
