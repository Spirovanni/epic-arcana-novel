...
/**
 * Merge detailed story information from Book_6_City_of_Shadows_Outline.json into l_outline.json
 * This script adds hero_journey_beat, character_arcs, story_gaps_addressed, 
 * location_details, and series_connections to all Book 6 chapters
 */
...
async function mergeBook6StoryDetails() {
  console.log('🚀 Starting merge of Book 6 story details');
  
  try {
    // Load the Book_6_City_of_Shadows_Outline.json file
    const book6OutlinePath = resolve(__dirname, '../lore/json/storyline/Book_6_City_of_Shadows_Outline.json');
    const book6Data: BookOutline = JSON.parse(readFileSync(book6OutlinePath, 'utf-8'));
    ...
    // Create a map of chapter data from Book_6_City_of_Shadows_Outline.json
    const chapterDataMap = new Map<number, Chapter>();
    
    book6Data.structure.forEach(section => {
    ...
    // Navigate to Book 6 in l_outline.json and add the missing fields
    ...
    // Find Book 6 in the structure
    let book6Found = false;
    ...
    for (const trilogyKey of Object.keys(trilogyBooks)) {
      ...
      if (trilogy.trilogy_books) {
        for (const [bookKey, bookData] of Object.entries(trilogy.trilogy_books as any)) {
          // Check if this is Book 6 (MT 6)
          if ((bookData as any).unique_identifier === 'MT 6') {
            book6Found = true;
            console.log(`✅ Found Book 6: ${(bookData as any).fiction_novel_title}`);
...
// Run the merge
mergeBook6StoryDetails().catch(console.error);