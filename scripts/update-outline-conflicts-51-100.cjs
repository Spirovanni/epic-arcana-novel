...
console.log('🔄 Updating internal_conflict fields in l_outline.json for scenes 51-100...\n');
...
let updatedCount = 0;
let sceneIndex = 0;
let targetSceneStart = 50; // Start at scene 51 (index 50)
  ...
  if (typeof obj === 'object' && obj !== null) {
    ...
    if (obj.scenes && Array.isArray(obj.scenes)) {
      obj.scenes.forEach((scene) => {
        // Only process scenes 51-100 (indices 50-99)
...
if (updatedCount > 0) {
  ...
  console.log(`   (Processed scenes 51-100, total ${sceneIndex} scenes in outline)`);
} else {
