...
/**
 * Merge detailed story information from Book_1_Only_Outline.json into l_outline.json
 * This script adds hero_journey_beat, character_arcs, story_gaps_addressed, 
 * location_details, and series_connections to all Book 1 chapters
 */
...
async function mergeBook1StoryDetails() {
  console.log('🚀 Starting merge of Book 1 story details');
  
  try {
    // Load the Book_1_Only_Outline.json file
    const book1OutlinePath = resolve(__dirname, '../lore/json/storyline/Book_1_Only_Outline.json');
    const book1Data: BookOutline = JSON.parse(readFileSync(book1OutlinePath, 'utf-8'));
    ...
    // Create a map of chapter data from Book_1_Only_Outline.json
    const chapterDataMap = new Map<number, Chapter>();
    
    book1Data.structure.forEach(section => {
    ...
    // Navigate to Book 1 in l_outline.json and add the missing fields
    ...
    for (const trilogyKey of Object.keys(trilogyBooks)) {
      ...
      if (trilogy.trilogy_books) {
        for (const [bookKey, bookData] of Object.entries(trilogy.trilogy_books as any)) {
          // Check if this is Book 1 (MT 1)
          if ((bookData as any).unique_identifier === 'MT 1') {
            book1Found = true;
            console.log(`✅ Found Book 1: ${(bookData as any).fiction_novel_title}`);
                ...
                if ((taskMaster as any).major_task_groups) {
                  for (const [majorTaskGroupKey, majorTaskGroup] of Object.entries((taskMaster as any).major_task_groups)) {
                    if ((majorTaskGroup as any).specific_task_groups) {
                      for (const [specificTaskGroupKey, specificTaskGroup] of Object.entries((majorTaskGroup as any).specific_task_groups)) {
...
// Run the merge
mergeBook1StoryDetails().catch(console.error);