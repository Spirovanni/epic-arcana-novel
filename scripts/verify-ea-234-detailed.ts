...
async function verifyEA234() {
    console.log('🔍 Verifying EA-234 scenes in database...\n');

    const chapterNumber = 234;
    ...
    `;
        ...
        if (chapters.length === 0) {
            console.log('❌ Error: EA-234 chapter not found in database');
            process.exit(1);
        }
    ...
    `;
        ...
        console.log(`\n${'='.repeat(60)}`);
        console.log(`📊 Summary for EA-234:`);
        ...
        if (allFieldsPopulated && scenes.length === expectedSceneCount) {
            console.log('✅ Verification PASSED - EA-234 is complete and correct!');
            process.exit(0);
        } else {
