...
async function main() {
  console.log('🔍 Verifying EA-142...\n');
  const [chapter] = await db.select().from(chapters).where(eq(chapters.chapterNumber, 142)).limit(1);
  if (!chapter) { console.log('❌ Chapter 142 not found'); return; }
