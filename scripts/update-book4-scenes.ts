...
function determineCoreEmotion(title: string, summary: string): string {
...
function determineSceneTone(title: string, summary: string): string {
  const content = (title + ' ' + summary).toLowerCase();
...
function createScenesForChapter(chapter: Book4Chapter): Scene[] {
...
function processBook4Data(): Map<number, Scene[]> {
  const book4Path = '/Users/xaviermartinez/dev/cursor/epic-arcana-novel/lore/json/storyline/Book_4_Tome_of_Fates_Outline.json';
  const book4Data: Book4Data = JSON.parse(fs.readFileSync(book4Path, 'utf-8'));
  
  const chapterScenesMap = new Map<number, Scene[]>();
...
function updateOutlineWithScenes() {
  ...
  function updateChapterScenes(obj: any, currentPath: string = '') {
  ...
  console.log('Successfully updated l_outline.json with Book 4 scenes');
}
