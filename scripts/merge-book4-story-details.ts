...
/**
 * Merge detailed story information from Book_4_Tome_of_Fates_Outline.json into l_outline.json
 * This script adds hero_journey_beat, character_arcs, story_gaps_addressed, 
 * location_details, and series_connections to all Book 4 chapters
 */
...
async function mergeBook4StoryDetails() {
  console.log('🚀 Starting merge of Book 4 story details');
  
  try {
    // Load the Book_4_Tome_of_Fates_Outline.json file
    const book4OutlinePath = resolve(__dirname, '../lore/json/storyline/Book_4_Tome_of_Fates_Outline.json');
    const book4Data: BookOutline = JSON.parse(readFileSync(book4OutlinePath, 'utf-8'));
    ...
    // Create a map of chapter data from Book_4_Tome_of_Fates_Outline.json
    ...
    // Navigate to Book 4 in l_outline.json and add the missing fields
    ...
    // Find Book 4 in the structure
    let book4Found = false;
    ...
    for (const trilogyKey of Object.keys(trilogyBooks)) {
      ...
      if (trilogy.trilogy_books) {
        for (const [bookKey, bookData] of Object.entries(trilogy.trilogy_books as any)) {
          // Check if this is Book 4 (MT 4)
          if ((bookData as any).unique_identifier === 'MT 4') {
            book4Found = true;
            console.log(`✅ Found Book 4: ${(bookData as any).fiction_novel_title}`);
...
// Run the merge
mergeBook4StoryDetails().catch(console.error);