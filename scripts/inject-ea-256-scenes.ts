...
async function main() {
  ...
  const enhancedScenesPath = path.join(process.cwd(), 'scripts', 'ea-256-enhanced-scenes.json');
  ...
  const backupPath = path.join(process.cwd(), 'data', 'l_outline.backup-ea-256.json');
  fs.writeFileSync(backupPath, JSON.stringify(outline, null, 2), 'utf-8');
  console.log(`💾 Backup: data/l_outline.backup-ea-256.json\n`);
