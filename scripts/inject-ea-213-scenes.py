#!/usr/bin/env python3
"""
Safe injection script for EA-213 enhanced scenes into l_outline.json
...
def inject_scenes_for_chapter(outline_path, scenes_path, chapter_id):
    ...
    Args:
        ...
        chapter_id: Chapter identifier (e.g., "EA-213")
    """
    # Load outline
        ...
        for book_key, book in trilogy_books.items():
            ...
            for tm_key, tm in task_masters.items():
                ...
                for mtg_key, mtg in major_task_groups.items():
                    # EA-213 is a Major Activity Theme, check both locations
                    major_activity_themes = mtg.get('major_activity_themes', {})
                    for mat_key, mat in major_activity_themes.items():
                        if mat.get('id') == chapter_id:
                            print(f"Found {chapter_id} at path: {trilogy_key} > {book_key} > {tm_key} > {mtg_key} > {mat_key}")

                            # Check if scenes field exists
                            if 'scenes' not in mat:
                                print(f"Creating new scenes array for {chapter_id}")
                                mat['scenes'] = []

                            existing_scenes = mat.get('scenes', [])
                            print(f"Existing scenes: {len(existing_scenes)}")
                            print(f"New scenes: {len(new_scenes)}")

                            # Update scenes array
                            mat['scenes'] = new_scenes
                            found = True
                    ...
                    # Also check Specific_task_groups in case structure differs
...
if __name__ == "__main__":
    ...
    scenes_path = project_root / "scripts" / "ea-213-enhanced-scenes.json"
    chapter_id = "EA-213"

    print(f"EA-213 Scene Injection Script")
    print(f"=" * 50)
