...
async function verify() {
  ...
  const chapter57 = await db.select({
    id: chapters.id,
  ...
  }).from(chapters).where(and(eq(chapters.bookId, book2[0].id), eq(chapters.chapterNumber, 57))).limit(1);
  
  if (chapter57.length === 0) {
    console.log('❌ Chapter EA-057 (Chapter 57, Book 2) not found');
    process.exit(1);
  }
  
  const ch = chapter57[0];
  
  console.log('✅ EA-057 Chapter 57 (Book 2, Chapter 17) Verification:');
