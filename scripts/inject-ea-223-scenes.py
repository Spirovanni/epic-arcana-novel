#!/usr/bin/env python3
"""
Safe injection script for EA-223 enhanced scenes into l_outline.json
EA-223 is a Major Activity Theme, so path differs from Specific Task Groups.
...
def inject_scenes_for_chapter(outline_path, scenes_path, chapter_id):
    """
    Safely inject scenes for a Major Activity Theme chapter, preserving all metadata.

    Args:
        outline_path: Path to l_outline.json
        scenes_path: Path to enhanced-scenes.json
        chapter_id: Chapter identifier (e.g., "EA-223")
    """
    # Load outline
...
if __name__ == "__main__":
    ...
    scenes_path = project_root / "scripts" / "ea-223-enhanced-scenes.json"
    chapter_id = "EA-223"

    print(f"EA-223 Scene Injection Script (Major Activity Theme)")
    print(f"=" * 50)
