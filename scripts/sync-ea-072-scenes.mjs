/**
 * Sync EA-072 Scene Updates to Neon DB
 * 
 * This script reads EA-072 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (72) → DB chapter_number (72) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 32") → Book 2's 32nd chapter
 * 
 * Usage: node scripts/sync-ea-072-scenes.mjs
 */
...
async function syncEA072Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-072 scenes to database\n');

        // Find EA-072 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-072');

        if (!eaData) {
            throw new Error('EA-072 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-072: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-072');
        }
