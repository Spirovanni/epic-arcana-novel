/**
 * Sync EA-061 Scene Updates to Neon DB
 * 
 * This script reads EA-061 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (61) → DB chapter_number (61) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 21") → Book 2's 21st chapter
 * 
 * Usage: node scripts/sync-ea-061-scenes.mjs
 */
...
async function syncEA061Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-061 scenes to database\n');

        // Find EA-061 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-061');

        if (!eaData) {
            throw new Error('EA-061 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-061: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-061');
        }
