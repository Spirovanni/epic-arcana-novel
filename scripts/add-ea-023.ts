...
/**
 * Add EA-023 chapter from l_outline.json to Neon DB
 */
...
function findEa023(data: any): any {
...
async function addEa023() {
  console.log('🔍 Finding EA-023 in l_outline.json...\n');
  ...
  try {
    ...
    const chapterData = findEa023(outlineData);
    
    if (!chapterData) {
      console.log('❌ EA-023 not found in l_outline.json');
      process.exit(1);
    }
    
    console.log('✅ Found EA-023:');
    console.log(`   Title: ${chapterData.major_task_group_title || chapterData.title}`);
    ...
    // Get book
    const bookRecord = await db.select({ id: books.id }).from(books).where(eq(books.bookNumber, chapterData.novel_book)).limit(1);
    
    if (bookRecord.length === 0) {
      console.log(`❌ Book ${chapterData.novel_book} not found in database`);
      process.exit(1);
    }
    ...
    // Note: EA-023 is a MAT chapter, so it uses major_task_group_title instead of specific_task_group_title
    const chapterDbData = {
      chapterId: chapterData.id, // EA-023
      uniqueIdentifier: chapterData.unique_identifier.startsWith('EA-') ? null : chapterData.unique_identifier, // MAT 1.2
      title: chapterData.major_task_group_title || chapterData.title,
      ...
      description: chapterData.major_task_group_description || chapterData.description,
      ...
      specificTaskGroupDescription: chapterData.major_task_group_description,
      specificTaskGroupTagline: chapterData.major_task_group_tagline || chapterData.tagline,
      // MAT chapters use major_activity_theme_books_influenced_by instead of specific_task_group_books_influenced_by
      specificTaskGroupBooksInfluencedBy: sanitizeJsonData(chapterData.major_activity_theme_books_influenced_by || chapterData.specific_task_group_books_influenced_by),
    ...
    console.log('\n📊 Chapter EA-023 successfully added/updated in Neon DB!');
    
  } catch (error) {
    console.error('\n❌ Error adding EA-023:', error);
    process.exit(1);
  }
}

addEa023().catch(console.error);
