...
async function main() {
  console.log('🔍 Verifying EA-151...\n');
  
  const [chapter] = await db.select().from(chapters).where(eq(chapters.chapterNumber, 151)).limit(1);
  if (!chapter) {
    console.log('❌ Chapter 151 not found');
    return;
  }
  ...
  console.log(`📖 Chapter 151: ${chapter.title}`);
