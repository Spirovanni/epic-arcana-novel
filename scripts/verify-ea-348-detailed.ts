...
// 34 required columns (DB schema compliant)
...
async function verifyEA348() {
    console.log('🔍 Verifying comprehensive EA-348 scenes...\n');

    // 1. Get Chapter
    const chapters = await sql`
    SELECT id, title 
    FROM chapters 
    WHERE chapter_number = 348
  `;

    if (chapters.length === 0) {
        console.error('❌ EA-348 (Chapter 348) not found!');
        process.exit(1);
    }
  ...
  `;
    ...
    for (const scene of scenes) {
        ...
        // Allow chronological_sequence checked separately as number
        ...
        const citationTerms = ['Lencioni', 'McChrystal', 'Sinek'];
    ...
    if (allPassed) {
        console.log('============================================================');
        console.log('✅ EA-348 DETAILED VERIFICATION PASSED!');
