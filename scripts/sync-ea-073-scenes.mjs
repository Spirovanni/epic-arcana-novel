/**
 * Sync EA-073 Scene Updates to Neon DB
 * 
 * This script reads EA-073 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (73) → DB chapter_number (73) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 33") → Book 2's 33rd chapter
 * 
 * Usage: node scripts/sync-ea-073-scenes.mjs
 */
...
async function syncEA073Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-073 scenes to database\n');

        // Find EA-073 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-073');

        if (!eaData) {
            throw new Error('EA-073 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-073: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-073');
        }
