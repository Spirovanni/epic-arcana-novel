...
console.log('🔄 Updating internal_conflict fields in l_outline.json for scenes 101-150...\n');

// Read the analysis file
const analysisPath = path.join(__dirname, 'scenes-101-150-analysis.json');
...
let updatedCount = 0;
let sceneIndex = 0;
let targetSceneStart = 100; // Start at scene 101 (index 100)
  ...
  if (typeof obj === 'object' && obj !== null) {
    ...
    if (obj.scenes && Array.isArray(obj.scenes)) {
      obj.scenes.forEach((scene) => {
        // Only process scenes 101-150 (indices 100-149)
...
if (updatedCount > 0) {
  ...
  console.log(`   (Processed scenes 101-150, total ${sceneIndex} scenes in outline)`);
} else {
