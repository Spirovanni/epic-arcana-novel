/**
 * Sync EA-066 Scene Updates to Neon DB
 * 
 * This script reads EA-066 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (66) → DB chapter_number (66) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 26") → Book 2's 26th chapter
 * 
 * Usage: node scripts/sync-ea-066-scenes.mjs
 */
...
async function syncEA066Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-066 scenes to database\n');

        // Find EA-066 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-066');

        if (!eaData) {
            throw new Error('EA-066 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-066: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-066');
        }
