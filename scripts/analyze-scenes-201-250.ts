...
// Extract scenes 201-250
const targetScenes = allScenes.filter(s => s.global_index >= 201 && s.global_index <= 250);

console.log(`Scenes 201-250: ${targetScenes.length} scenes`);

if (targetScenes.length === 0) {
  console.log('No scenes found in range 201-250!');
  process.exit(1);
}
...
function extractCharacters(scene: Scene & { chapter_id: string }): string[] {
  ...
  const knownCharacters = [
    ...
    'Council', 'Team', 'Board', 'Temporal Monks'
  ];
...
const outputPath = path.join(__dirname, 'scenes-201-250-analysis.json');
