/**
 * Sync EA-074 Scene Updates to Neon DB
 * 
 * This script reads EA-074 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (74) → DB chapter_number (74) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 34") → Book 2's 34th chapter
 * 
 * Usage: node scripts/sync-ea-074-scenes.mjs
 */
...
async function syncEA074Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-074 scenes to database\n');

        // Find EA-074 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-074');

        if (!eaData) {
            throw new Error('EA-074 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-074: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-074');
        }
