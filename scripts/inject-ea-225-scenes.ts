...
/**
 * Safely inject enhanced scenes for EA-225 into l_outline.json
 */
...
async function main() {
  ...
  const enhancedScenesPath = path.join(process.cwd(), 'scripts', 'ea-225-enhanced-scenes.json');
  const enhancedScenes = JSON.parse(fs.readFileSync(enhancedScenesPath, 'utf-8'));
  console.log(`✅ Loaded ${enhancedScenes.length} enhanced scenes from ea-225-enhanced-scenes.json\n`);
  ...
  // Find and update EA-225 in the outline
  ...
  // Create backup
  const backupPath = path.join(process.cwd(), 'data', 'l_outline.backup-ea-225.json');
  fs.writeFileSync(backupPath, JSON.stringify(outline, null, 2), 'utf-8');
  console.log(`💾 Created backup at data/l_outline.backup-ea-225.json\n`);
