/**
 * Sync EA-049 Scene Updates to Neon DB
 * 
 * This script reads EA-049 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * Usage: node scripts/sync-ea-049-scenes.mjs
 */
...
async function syncEA049Scenes() {
  try {
    ...
    console.log('\n🎯 Syncing EA-049 scenes to database\n');
    
    // Find EA-049 in the outline structure
    ...
    const ea049Data = findChapterById(outlineData, 'EA-049');
    
    if (!ea049Data) {
      throw new Error('EA-049 not found in data/l_outline.json');
    }
    
    console.log(`📚 Found EA-049: ${ea049Data.specific_task_group_title || ea049Data.title}`);
    console.log(`   Chapter Number: ${ea049Data.all_chapter}`);
    console.log(`   Novel Book: ${ea049Data.novel_book}`);
    console.log(`   Unique ID: ${ea049Data.unique_identifier}`);
    console.log(`   Scenes: ${ea049Data.scenes?.length || 0}`);
    
    if (!ea049Data.scenes || ea049Data.scenes.length === 0) {
      throw new Error('No scenes found in EA-049');
    }
    ...
    let chapterQuery = await client.query(
      `SELECT id, chapter_number, title, unique_identifier 
       ...
       LIMIT 1`,
      [ea049Data.unique_identifier]
    );
    
    if (chapterQuery.rows.length === 0) {
      console.log(`   Trying to find by chapter_number: ${ea049Data.all_chapter}`);
      chapterQuery = await client.query(
        `SELECT id, chapter_number, title, unique_identifier 
         FROM chapters 
         WHERE chapter_number = $1 
         LIMIT 1`,
        [ea049Data.all_chapter]
      );
    }
    
    if (chapterQuery.rows.length === 0) {
      throw new Error(
        `Chapter not found in database. Please create chapter ${ea049Data.all_chapter} ` +
        `(unique_id: ${ea049Data.unique_identifier}) before syncing scenes.`
      );
    }
    ...
    console.log(`\n📝 Processing ${ea049Data.scenes.length} scenes...\n`);
    
    for (const scene of ea049Data.scenes) {
    ...
    console.log(`🎯 Chapter: ${chapter.chapter_number} - ${ea049Data.specific_task_group_title}`);
    console.log('='.repeat(60));
