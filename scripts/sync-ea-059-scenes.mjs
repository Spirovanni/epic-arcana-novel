/**
 * Sync EA-059 Scene Updates to Neon DB
 * 
 * This script reads EA-059 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (59) → DB chapter_number (59) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 19") → Book 2's 19th chapter
 * 
 * Usage: node scripts/sync-ea-059-scenes.mjs
 */
...
async function syncEA059Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-059 scenes to database\n');

        // Find EA-059 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-059');

        if (!eaData) {
            throw new Error('EA-059 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-059: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-059');
        }
