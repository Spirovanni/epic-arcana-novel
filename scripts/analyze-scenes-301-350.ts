...
// Extract scenes 301-350
const targetScenes = allScenes.filter(s => s.global_index >= 301 && s.global_index <= 350);

console.log(`Scenes 301-350: ${targetScenes.length} scenes`);

if (targetScenes.length === 0) {
  console.log('No scenes found in range 301-350!');
  process.exit(0);
}
...
const outputPath = path.join(__dirname, 'scenes-301-350-analysis.json');
...
analysis.slice(0, 3).forEach(a => {
  ...
  console.log(`\nCurrent conflict: ${a.current_internal_conflict.substring(0, 150)}...`);
  console.log(`\nProposed conflict: ${a.proposed_enhanced_conflict.substring(0, 150)}...`);
});
