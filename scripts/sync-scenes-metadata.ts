...
async function syncScenesMetadata() {
  try {
    ...
    // Process chapters 1-4 from task_master_1
    for (const mtgKey in taskMaster1) {
      if (mtgKey === 'major_activity_theme_1') continue;
      ...
      for (const stgKey in majorGroup.Specific_task_groups) {
        ...
        console.log(`\nProcessing Chapter ${chapterNum} (${specificGroup.title})`);
        ...
        for (const scene of scenesList) {
          ...
          const chapterResult = await client.query(
            'SELECT id FROM chapters WHERE chapter_number = $1 LIMIT 1',
            [chapterNum]
          );
    ...
    // Process task_master_2 (Chapters 5-6)
