...
/**
 * Add EA-063 chapter from l_outline.json to Neon DB
 * Book 2, Chapter 23 (all_chapter 63)
 */
...
function findEa063(data: any): any {
...
async function addEa063() {
  console.log('🔍 Finding EA-063 in l_outline.json...\n');
  ...
  try {
    ...
    const chapterData = findEa063(outlineData);
    
    if (!chapterData) {
      console.log('❌ EA-063 not found in l_outline.json');
      process.exit(1);
    }
    
    console.log('✅ Found EA-063:');
    ...
    const chapterDbData = {
      chapterId: chapterData.id, // EA-063
      uniqueIdentifier: chapterData.unique_identifier.startsWith('EA-') ? null : chapterData.unique_identifier, // MAT 2.2
      title: title,
    ...
    console.log('\n📊 Chapter EA-063 successfully added/updated in Neon DB!');
    
  } catch (error) {
    console.error('\n❌ Error adding EA-063:', error);
    process.exit(1);
  }
}

addEa063().catch(console.error);
