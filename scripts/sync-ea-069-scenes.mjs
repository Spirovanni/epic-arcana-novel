/**
 * Sync EA-069 Scene Updates to Neon DB
 * 
 * This script reads EA-069 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (69) → DB chapter_number (69) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 29") → Book 2's 29th chapter
 * 
 * Usage: node scripts/sync-ea-069-scenes.mjs
 */
...
async function syncEA069Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-069 scenes to database\n');

        // Find EA-069 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-069');

        if (!eaData) {
            throw new Error('EA-069 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-069: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-069');
        }
