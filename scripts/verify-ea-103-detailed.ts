...
async function main() {
  console.log('🔍 Detailed verification of EA-103 scenes...\n');

  const [chapter] = await db
    .select()
    .from(chapters)
    .where(eq(chapters.chapterNumber, 103))
    .limit(1);

  if (!chapter) {
    console.log('❌ Chapter 103 not found');
    return;
  }
  ...
  for (const scene of chapterScenes) {
    ...
    console.log(`   Sensory Detail: ${scene.sensoryDetail ? scene.sensoryDetail.substring(0, 80) + '...' : 'N/A'}`);
    ...
    console.log(`   Narrative Function: ${scene.narrativeFunction || 'N/A'}`);
