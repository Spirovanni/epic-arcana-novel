...
const BOOK4_JSON_PATH = path.join(process.cwd(), 'lore', 'book4_expanded_codex.json');
...
async function seedBook4Data() {
  ...
  console.log('Loading book4 expanded codex data...');
  const book4Data = await loadJSON(BOOK4_JSON_PATH);

  console.log('Starting book4 expanded codex seeding...');
  ...
  // Check if book4 already exists
  const existingBook = await db.select().from(schema.books).where(eq(schema.books.bookNumber, 4));
  let bookId;

  if (existingBook.length === 0) {
    console.log('Creating new book4 record...');
    const newBook = await db.insert(schema.books).values({
      seriesId: seriesId,
      bookNumber: 4,
      uniqueIdentifier: book4Data.book_unique_identifier,
      title: book4Data.fantasy_book_name,
      subject: book4Data.book_subject,
      focus: book4Data.focus_of_book,
      description: book4Data.book_description,
      themes: [book4Data.unique_theme],
      enneagramType: book4Data.enneagram,
      militaryComponent: book4Data.military,
      businessModel: book4Data.business_model_generation,
      personalityType: book4Data.personality_type,
      coveryCoveyHabit: book4Data['9_habits_covey'],
      associatedSin: book4Data.sin,
    }).returning({ id: schema.books.id });
    bookId = newBook[0].id;
  } else {
    bookId = existingBook[0].id;
    console.log(`Using existing book4 ID: ${bookId}`);
  ...
  for (const section of book4Data.sections) {
  ...
  console.log(`Seeding completed! Created ${chapterCount} chapters for book4.`);
  await pool.end();
}
...
async function main() {
  try {
    await seedBook4Data();
    console.log('Book4 expanded codex seeding completed successfully!');
  } catch (error) {
