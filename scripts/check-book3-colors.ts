...
/**
 * Script to check current Book 3 colors in the database
 */
...
async function checkBook3Colors() {
  try {
    // Get Book 3 first
    ...
    console.log(`📚 Found Book 3: ${book3[0].title} (ID: ${book3[0].id})`);

    // Get chapters for Book 3
    const bookChapters = await db.select({
      ...
      .where(eq(chapters.bookId, book3[0].id))
    ...
    console.error('❌ Error checking Book 3 colors:', error);
  }
}
