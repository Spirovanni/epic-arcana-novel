...
const BOOK3_JSON_PATH = path.join(process.cwd(), 'lore', 'book3_expanded_codex.json');
...
async function seedBook3Data() {
  ...
  console.log('Loading book3 expanded codex data...');
  const book3Data = await loadJSON(BOOK3_JSON_PATH);

  console.log('Starting book3 expanded codex seeding...');
  ...
  // Check if book3 already exists
  const existingBook = await db.select().from(schema.books).where(eq(schema.books.bookNumber, 3));
  let bookId;

  if (existingBook.length === 0) {
    console.log('Creating new book3 record...');
    const newBook = await db.insert(schema.books).values({
      seriesId: seriesId,
      bookNumber: 3,
      uniqueIdentifier: book3Data.book_unique_identifier,
      title: book3Data.fantasy_book_name,
      subject: book3Data.book_subject,
      focus: book3Data.focus_of_book,
      description: book3Data.book_description,
      themes: [book3Data.unique_theme],
      enneagramType: book3Data.enneagram,
      militaryComponent: book3Data.military,
      businessModel: book3Data.business_model_generation,
      personalityType: book3Data.personality_type,
      coveryCoveyHabit: book3Data['9_habits_covey'],
      associatedSin: book3Data.sin,
    }).returning({ id: schema.books.id });
    bookId = newBook[0].id;
  } else {
    bookId = existingBook[0].id;
    console.log(`Using existing book3 ID: ${bookId}`);
  ...
  for (const section of book3Data.sections) {
  ...
  console.log(`Seeding completed! Created ${chapterCount} chapters for book3.`);
  await pool.end();
}
...
async function main() {
  try {
    await seedBook3Data();
    console.log('Book3 expanded codex seeding completed successfully!');
  } catch (error) {
