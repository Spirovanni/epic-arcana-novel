...
async function main() {
  console.log('🔍 Searching for EA-026 in outline...\n');
  ...
  console.log(`✅ Found EA-026: ${ea026Data.title}`);
  console.log(`   Scenes: ${ea026Data.scenes?.length || 0}\n`);

  // Find the chapter - it should be chapter 26 in the same book as EA-023
  ...
  if (existingChapter.length > 0) {
    console.log('✅ EA-026 chapter already exists\n');
    chapter = existingChapter[0];
  } else {
    // Find chapter 26 in the same book
    const ch26 = await db
      .select()
      .from(chapters)
      .where(eq(chapters.chapterNumber, 26))
      .limit(10);

    const correctChapter = ch26.find((ch) => ch.bookId === bookId);

    if (!correctChapter) {
      console.error('❌ Chapter 26 not found in the same book as EA-023');
      process.exit(1);
    }

    chapter = correctChapter;
    console.log(`✅ Found Chapter 26: ${chapter.title}\n`);

    // Update with unique identifier and outline data
    await db
      .update(chapters)
      .set({
        uniqueIdentifier: 'EA-026',
        title: ea026Data.title,
      })
      .where(eq(chapters.id, chapter.id));

    console.log('✅ Updated chapter with EA-026 identifier\n');
  }
  ...
  for (const sceneData of ea026Data.scenes || []) {
    const sceneInsert = {
      chapterId: chapter.id,
      chapterUniqueIdentifier: 'EA-026',
