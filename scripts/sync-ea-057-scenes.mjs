/**
 * Sync EA-057 Scene Updates to Neon DB
 * 
 * This script reads EA-057 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (57) → DB chapter_number (57) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 17") → Book 2's 17th chapter
 * 
 * Usage: node scripts/sync-ea-057-scenes.mjs
 */
...
async function syncEA057Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-057 scenes to database\n');

        // Find EA-057 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-057');

        if (!eaData) {
            throw new Error('EA-057 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-057: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-057');
        }

        // Get Book 2 ID first
        // We assume Book 2 based on context, but let's check eaData.novel_book
        // If eaData.novel_book is string "2", it should match.
        const bookNumber = parseInt(eaData.novel_book) || 2;
       ...
       LIMIT 1`,
        ...
        console.log(`🎯 Chapter: ${eaData.all_chapter} (Book ${bookNumber}, Ch ${eaData.chapter.replace('Chapter ', '')}) - ${eaData.specific_task_group_title}`);
        console.log('='.repeat(70));
