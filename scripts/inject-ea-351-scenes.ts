...
async function main() {
  ...
  const backupPath = path.join(__dirname, `../data/l_outline.json.backup-${eaId.toLowerCase()}`);
  ...
  console.log(`\n📖 Importing scenes for ${eaId} (Chapter ${chapterNumber})...`);
  ...
  for (const scene of enhancedScenes) {
    ...
    await db.update(scenes)
      .set({
        ...
        sensoryDetail: scene.sensory_detail,
        internalConflict: scene.internal_conflict,
        pages: scene.pages,
        temporalPowerManifested: scene.temporal_power_manifested,
        storySequence: scene.story_sequence,
      })
