/**
 * Sync EA-058 Scene Updates to Neon DB
 * 
 * This script reads EA-058 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (58) → DB chapter_number (58) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 18") → Book 2's 18th chapter
 * 
 * Usage: node scripts/sync-ea-058-scenes.mjs
 */
...
async function syncEA058Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-058 scenes to database\n');

        // Find EA-058 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-058');

        if (!eaData) {
            throw new Error('EA-058 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-058: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-058');
        }

        // Get Book 2 ID first
        // Note: EA-058 might be missing 'novel_book' in some JSON versions, default to 2
