...
const BOOK8_JSON_PATH = path.join(process.cwd(), 'lore', 'book8_expanded_codex.json');
...
async function seedBook8Data() {
  ...
  console.log('Loading book8 expanded codex data...');
  const book8Data = await loadJSON(BOOK8_JSON_PATH);

  console.log('Starting book8 expanded codex seeding...');
  ...
  // Check if book8 already exists
  const existingBook = await db.select().from(schema.books).where(eq(schema.books.bookNumber, 8));
  let bookId;

  if (existingBook.length === 0) {
    console.log('Creating new book8 record...');
    const newBook = await db.insert(schema.books).values({
      seriesId: seriesId,
      bookNumber: 8,
      uniqueIdentifier: book8Data.book_unique_identifier,
      title: book8Data.fantasy_book_name,
      subject: book8Data.book_subject,
      focus: book8Data.focus_of_book,
      description: book8Data.book_description,
      themes: [book8Data.unique_theme],
      enneagramType: book8Data.enneagram,
      militaryComponent: book8Data.military,
      businessModel: book8Data.business_model_generation,
      personalityType: book8Data.personality_type,
      coveryCoveyHabit: book8Data['9_habits_covey'],
      associatedSin: book8Data.sin,
    }).returning({ id: schema.books.id });
    bookId = newBook[0].id;
  } else {
    bookId = existingBook[0].id;
    console.log(`Using existing book8 ID: ${bookId}`);
  ...
  for (const section of book8Data.sections) {
  ...
  console.log(`Seeding completed! Created ${chapterCount} chapters for book8.`);
  await pool.end();
}
...
async function main() {
  try {
    await seedBook8Data();
    console.log('Book8 expanded codex seeding completed successfully!');
  } catch (error) {
