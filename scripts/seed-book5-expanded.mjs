...
const BOOK5_JSON_PATH = path.join(process.cwd(), 'lore', 'book5_expanded_codex.json');
...
async function seedBook5Data() {
  ...
  console.log('Loading book5 expanded codex data...');
  const book5Data = await loadJSON(BOOK5_JSON_PATH);

  console.log('Starting book5 expanded codex seeding...');
  ...
  // Check if book5 already exists
  const existingBook = await db.select().from(schema.books).where(eq(schema.books.bookNumber, 5));
  let bookId;

  if (existingBook.length === 0) {
    console.log('Creating new book5 record...');
    const newBook = await db.insert(schema.books).values({
      seriesId: seriesId,
      bookNumber: 5,
      uniqueIdentifier: book5Data.book_unique_identifier,
      title: book5Data.fantasy_book_name,
      subject: book5Data.book_subject,
      focus: book5Data.focus_of_book,
      description: book5Data.book_description,
      themes: [book5Data.unique_theme],
      enneagramType: book5Data.enneagram,
      militaryComponent: book5Data.military,
      businessModel: book5Data.business_model_generation,
      personalityType: book5Data.personality_type,
      coveryCoveyHabit: book5Data['9_habits_covey'],
      associatedSin: book5Data.sin,
    }).returning({ id: schema.books.id });
    bookId = newBook[0].id;
  } else {
    bookId = existingBook[0].id;
    console.log(`Using existing book5 ID: ${bookId}`);
  ...
  for (const section of book5Data.sections) {
  ...
  console.log(`Seeding completed! Created ${chapterCount} chapters for book5.`);
  await pool.end();
}
...
async function main() {
  try {
    await seedBook5Data();
    console.log('Book5 expanded codex seeding completed successfully!');
  } catch (error) {
