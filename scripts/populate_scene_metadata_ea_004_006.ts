 ...
 */
async function populateSceneMetadata() {
    console.log('🎬 Starting Scene Metadata Population for EA-004, EA-005, EA-006 (STG Format)\n');
    ...
    try {
        ...
        // Target chapters - mapping EA-IDs to actual DB identifiers (STG format)
        ...
        for (const target of targetChapters) {
            console.log(`\n${'─'.repeat(70)}`);
            console.log(`📚 Processing Chapter: ${target.eaId} (${target.title})`);
            ...
            console.log(`✓ Found chapter data: ${chapterData.specific_task_group_title}`);
            console.log(`  Focus Area: ${chapterData.focus_area}`);
            console.log(`  Scenes in JSON: ${chapterData.scenes?.length || 0}`);

            // Find corresponding chapter in database using the STG identifier
            ...
            if (dbChapter.length === 0) {
                console.log(`⚠️  Chapter ${target.eaId} with DB ID ${target.dbId} not found in database, skipping...`);
                continue;
            }
            const chapterDbId = dbChapter[0].id;
            console.log(`✓ Found in database with ID: ${chapterDbId}`);

            // Process each scene
            const scenesArray = chapterData.scenes || [];
            console.log(`\n  Processing ${scenesArray.length} scenes...`);
            ...
            for (const sceneData of scenesArray) {
                try {
                    ...
                    const updateResult = await db
                        .update(scenes)
                        .set({
                            sudowrite_metadata: sudowriteMetadata,
                            learning_objectives: learningObjectives,
                            foreshadowing_elements: foreshadowingElements,
                            updatedAt: new Date()
                        })
                        .where(
                            and(
                                eq(scenes.chapterId, chapterDbId),
        ...
        console.log('\n' + '═'.repeat(70));
        console.log(`\n✅ Successfully updated ${totalScenesUpdated} scenes!`);
        console.log('\n📊 Summary:');
        console.log(`   - sudowrite_metadata: Populated`);
        console.log(`   - learning_objectives: Populated`);
        console.log(`   - foreshadowing_elements: Populated`);
