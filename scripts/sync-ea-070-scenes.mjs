/**
 * Sync EA-070 Scene Updates to Neon DB
 * 
 * This script reads EA-070 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (70) → DB chapter_number (70) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 30") → Book 2's 30th chapter
 * 
 * Usage: node scripts/sync-ea-070-scenes.mjs
 */
...
async function syncEA070Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-070 scenes to database\n');

        // Find EA-070 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-070');

        if (!eaData) {
            throw new Error('EA-070 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-070: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-070');
        }
