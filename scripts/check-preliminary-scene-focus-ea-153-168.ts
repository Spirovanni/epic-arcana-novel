...
/**
 * Check "preliminary_scene_focus" column format for EA-153 to EA-168
 */

async function main() {
  console.log(`🔍 Checking "preliminary_scene_focus" column format for EA-153 to EA-168...\n`);
  ...
  for (const chapter of allChapters) {
    ...
    for (const scene of chapterScenes) {
      ...
      const status = psf.startsWith('Daughter ') || psf.startsWith('Unexpected ') || psf.startsWith('Champion') ? '✅' : '❌';
      console.log(`   ${status} Scene ${scene.sceneNumber}: "${psf}"`);
    }
    console.log('');
  }
  ...
  console.log('Target chapters (153-168) will be fixed if needed.\n');
}
