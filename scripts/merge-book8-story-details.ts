...
/**
 * Merge detailed story information from Book_8_Splendor_of_Ancient_Garden_Outline.json into l_outline.json
 * This script adds hero_journey_beat, character_arcs, story_gaps_addressed, 
 * location_details, and series_connections to all Book 8 chapters
 */
...
async function mergeBook8StoryDetails() {
  console.log('🚀 Starting merge of Book 8 story details');
  
  try {
    // Load the Book_8_Splendor_of_Ancient_Garden_Outline.json file
    const book8OutlinePath = resolve(__dirname, '../lore/json/storyline/Book_8_Splendor_of_Ancient_Garden_Outline.json');
    const book8Data: BookOutline = JSON.parse(readFileSync(book8OutlinePath, 'utf-8'));
    ...
    // Create a map of chapter data from Book_8_Splendor_of_Ancient_Garden_Outline.json
    const chapterDataMap = new Map<number, Chapter>();
    
    book8Data.structure.forEach(section => {
    ...
    // Navigate to Book 8 in l_outline.json and add the missing fields
    ...
    // Find Book 8 in the structure
    let book8Found = false;
    ...
    for (const trilogyKey of Object.keys(trilogyBooks)) {
      ...
      if (trilogy.trilogy_books) {
        for (const [bookKey, bookData] of Object.entries(trilogy.trilogy_books as any)) {
          // Check if this is Book 8 (MT 8)
          if ((bookData as any).unique_identifier === 'MT 8') {
            book8Found = true;
            console.log(`✅ Found Book 8: ${(bookData as any).fiction_novel_title}`);
                          ...
                          if (sourceChapter) {
                            ...
                            if (!(specificTaskGroup as any).hero_journey_beat && sourceChapter.plot) {
                              ...
                              const plotToHeroJourney: Record<string, string> = {
                                ...
                                'Tests, Allies, Enemies': 'Tests, Allies, Enemies',
                                'Approach to the Inmost Cave': 'Approach to the Inmost Cave',
                                'The Ordeal': 'The Ordeal',
                                'Reward (Seizing the Sword)': 'Reward (Seizing the Sword)',
                                'The Road Back': 'The Road Back',
                                'Resurrection': 'Resurrection',
                                'Return with the Elixir': 'Return with the Elixir',
                            ...
                            if (!(specificTaskGroup as any).save_the_cat_beat && sourceChapter.plot) {
                              ...
                              const plotToSaveTheCat: Record<string, string> = {
                                ...
                                'Tests, Allies, Enemies': 'Fun and Games',
                                'Approach to the Inmost Cave': 'Midpoint',
                                'The Ordeal': 'Bad Guys Close In',
                                'Reward (Seizing the Sword)': 'All Is Lost',
                                'The Road Back': 'Dark Night of the Soul',
                                'Resurrection': 'Break into Three',
                                'Return with the Elixir': 'Finale',
                                'Intro': 'Opening Image',
...
// Run the merge
mergeBook8StoryDetails().catch(console.error);