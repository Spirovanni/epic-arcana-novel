/**
 * Sync EA-063 Scene Updates to Neon DB
 * 
 * This script reads EA-063 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (63) → DB chapter_number (63) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 23") → Book 2's 23rd chapter
 * 
 * Usage: node scripts/sync-ea-063-scenes.mjs
 */
...
async function syncEA063Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-063 scenes to database\n');

        // Find EA-063 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-063');

        if (!eaData) {
            throw new Error('EA-063 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-063: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-063');
        }
