/**
 * Sync EA-067 Scene Updates to Neon DB
 * 
 * This script reads EA-067 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (67) → DB chapter_number (67) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 27") → Book 2's 27th chapter
 * 
 * Usage: node scripts/sync-ea-067-scenes.mjs
 */
...
async function syncEA067Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-067 scenes to database\n');

        // Find EA-067 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-067');

        if (!eaData) {
            throw new Error('EA-067 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-067: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-067');
        }
