...
const BOOK6_JSON_PATH = path.join(process.cwd(), 'lore', 'book6_expanded_codex.json');
...
async function seedBook6Data() {
  ...
  console.log('Loading book6 expanded codex data...');
  const book6Data = await loadJSON(BOOK6_JSON_PATH);

  console.log('Starting book6 expanded codex seeding...');
  ...
  // Check if book6 already exists
  const existingBook = await db.select().from(schema.books).where(eq(schema.books.bookNumber, 6));
  let bookId;

  if (existingBook.length === 0) {
    console.log('Creating new book6 record...');
    const newBook = await db.insert(schema.books).values({
      seriesId: seriesId,
      bookNumber: 6,
      uniqueIdentifier: book6Data.book_unique_identifier,
      title: book6Data.fantasy_book_name,
      subject: book6Data.book_subject,
      focus: book6Data.focus_of_book,
      description: book6Data.book_description,
      themes: [book6Data.unique_theme],
      enneagramType: book6Data.enneagram,
      militaryComponent: book6Data.military,
      businessModel: book6Data.business_model_generation,
      personalityType: book6Data.personality_type,
      coveryCoveyHabit: book6Data['9_habits_covey'],
      associatedSin: book6Data.sin,
    }).returning({ id: schema.books.id });
    bookId = newBook[0].id;
  } else {
    bookId = existingBook[0].id;
    console.log(`Using existing book6 ID: ${bookId}`);
  ...
  for (const section of book6Data.sections) {
  ...
  console.log(`Seeding completed! Created ${chapterCount} chapters for book6.`);
  await pool.end();
}
...
async function main() {
  try {
    await seedBook6Data();
    console.log('Book6 expanded codex seeding completed successfully!');
  } catch (error) {
