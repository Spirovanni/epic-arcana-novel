...
/**
 * Updates existing EA-246 scenes with missing sensory_detail and internal_conflict fields
 */
...
async function main() {
    ...
    console.log(`🔄 Updating EA-246 scenes with missing fields...\n`);
    ...
    for (const outlineScene of chapterData.scenes) {
        ...
        // Update with missing fields (supporting both snake_case and camelCase)
        const updateData: any = {
            sensoryDetail: truncate(
                outlineScene.sensory_detail || outlineScene.sensoryDetail,
                500
            ),
            internalConflict: truncate(
                outlineScene.internal_conflict || outlineScene.internalConflict,
                255
            ),
        };

        await db.update(scenes).set(updateData).where(eq(scenes.id, existingScene.id));
        console.log(`   ✅ Updated with sensory_detail and internal_conflict\n`);
    }
