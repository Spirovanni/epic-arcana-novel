/**
 * Sync EA-064 Scene Updates to Neon DB
 * 
 * This script reads EA-064 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (64) → DB chapter_number (64) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 24") → Book 2's 24th chapter
 * 
 * Usage: node scripts/sync-ea-064-scenes.mjs
 */
...
async function syncEA064Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-064 scenes to database\n');

        // Find EA-064 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-064');

        if (!eaData) {
            throw new Error('EA-064 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-064: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-064');
        }
