...
/**
 * Merge detailed story information from Book_7_Dark_Path_to_Victory_Outline.json into l_outline.json
 * This script adds hero_journey_beat, character_arcs, story_gaps_addressed, 
 * location_details, and series_connections to all Book 7 chapters
 */
...
async function mergeBook7StoryDetails() {
  console.log('🚀 Starting merge of Book 7 story details');
  
  try {
    // Load the Book_7_Dark_Path_to_Victory_Outline.json file
    const book7OutlinePath = resolve(__dirname, '../lore/json/storyline/Book_7_Dark_Path_to_Victory_Outline.json');
    const book7Data: BookOutline = JSON.parse(readFileSync(book7OutlinePath, 'utf-8'));
    ...
    // Create a map of chapter data from Book_7_Dark_Path_to_Victory_Outline.json
    const chapterDataMap = new Map<number, Chapter>();
    
    book7Data.structure.forEach(section => {
    ...
    // Navigate to Book 7 in l_outline.json and add the missing fields
    ...
    // Find Book 7 in the structure
    let book7Found = false;
    ...
    for (const trilogyKey of Object.keys(trilogyBooks)) {
      ...
      if (trilogy.trilogy_books) {
        for (const [bookKey, bookData] of Object.entries(trilogy.trilogy_books as any)) {
          // Check if this is Book 7 (MT 7)
          if ((bookData as any).unique_identifier === 'MT 7') {
            book7Found = true;
            console.log(`✅ Found Book 7: ${(bookData as any).fiction_novel_title}`);
...
// Run the merge
mergeBook7StoryDetails().catch(console.error);