...
// Find and update the chapter - this one may not have a scenes array, so we add one
function findAndUpdateChapter(obj: any, targetId: string, newScenes: any[]): boolean {
  if (obj && typeof obj === 'object') {
    if (obj.id === targetId) {
      const existingScenes = obj.scenes?.length || 0;
      console.log(`Found chapter ${targetId} with ${existingScenes} existing scenes`);
      obj.scenes = newScenes;
      console.log(`Added/replaced with ${newScenes.length} enhanced scenes`);
      return true;
    }
