...
/**
 * Merge detailed story information from Book_9_Blades_of_Triumph_Outline.json into l_outline.json
 * This script adds hero_journey_beat, character_arcs, story_gaps_addressed, 
 * location_details, and series_connections to all Book 9 chapters
 */
...
async function mergeBook9StoryDetails() {
  console.log('🚀 Starting merge of Book 9 story details');
  
  try {
    // Load the Book_9_Blades_of_Triumph_Outline.json file
    const book9OutlinePath = resolve(__dirname, '../lore/json/storyline/Book_9_Blades_of_Triumph_Outline.json');
    const book9Data: BookOutline = JSON.parse(readFileSync(book9OutlinePath, 'utf-8'));
    ...
    // Create a map of chapter data from Book_9_Blades_of_Triumph_Outline.json
    ...
    // Navigate to Book 9 in l_outline.json and add the missing fields
    ...
    // Find Book 9 in the structure
    let book9Found = false;
    ...
    for (const trilogyKey of Object.keys(trilogyBooks)) {
      ...
      if (trilogy.trilogy_books) {
        for (const [bookKey, bookData] of Object.entries(trilogy.trilogy_books as any)) {
          // Check if this is Book 9 (MT 9)
          if ((bookData as any).unique_identifier === 'MT 9') {
            book9Found = true;
            console.log(`✅ Found Book 9: ${(bookData as any).fiction_novel_title}`);
...
// Run the merge
mergeBook9StoryDetails().catch(console.error);