/**
 * Sync EA-062 Scene Updates to Neon DB
 * 
 * This script reads EA-062 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (62) → DB chapter_number (62) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 22") → Book 2's 22nd chapter
 * 
 * Usage: node scripts/sync-ea-062-scenes.mjs
 */
...
async function syncEA062Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-062 scenes to database\n');

        // Find EA-062 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-062');

        if (!eaData) {
            throw new Error('EA-062 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-062: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-062');
        }
