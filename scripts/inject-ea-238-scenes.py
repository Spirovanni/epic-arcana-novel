#!/usr/bin/env python3
"""
Safe injection script for EA-238 enhanced scenes into l_outline.json
...
def inject_scenes_for_chapter(outline_path, scenes_path, chapter_id):
    ...
    Args:
        ...
        chapter_id: Chapter identifier (e.g., "EA-238")
    """
    # Load outline
        ...
        for book_key, book in trilogy_books.items():
            ...
            for tm_key, tm in task_masters.items():
                ...
                for mtg_key, mtg in major_task_groups.items():
                    # Check both Specific_task_groups and major_activity_themes
                    for group_type in ['Specific_task_groups', 'major_activity_themes']:
                        task_groups = mtg.get(group_type, {})
                        for stg_key, stg in task_groups.items():
                            if stg.get('id') == chapter_id:
                                print(f"Found {chapter_id} at path: {trilogy_key} > {book_key} > {tm_key} > {mtg_key} > {group_type} > {stg_key}")
...
if __name__ == "__main__":
    ...
    scenes_path = project_root / "scripts" / "ea-238-enhanced-scenes.json"
    chapter_id = "EA-238"

    print(f"EA-238 Scene Injection Script")
    print(f"=" * 50)
