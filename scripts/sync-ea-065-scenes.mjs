/**
 * Sync EA-065 Scene Updates to Neon DB
 * 
 * This script reads EA-065 from data/l_outline.json and upserts
 * the enhanced scene data into the Neon database.
 * 
 * CHAPTER NUMBERING:
 * - all_chapter (65) → DB chapter_number (65) [absolute chapter in series]
 * - novel_book (2) → DB book_id (Book 2) [book association]
 * - chapter ("Chapter 25") → Book 2's 25th chapter
 * 
 * Usage: node scripts/sync-ea-065-scenes.mjs
 */
...
async function syncEA065Scenes() {
    try {
        ...
        console.log('\n🎯 Syncing EA-065 scenes to database\n');

        // Find EA-065 in the outline structure
        ...
        const eaData = findChapterById(outlineData, 'EA-065');

        if (!eaData) {
            throw new Error('EA-065 not found in data/l_outline.json');
        }

        console.log(`📚 Found EA-065: ${eaData.specific_task_group_title || eaData.title}`);
        ...
        if (!eaData.scenes || eaData.scenes.length === 0) {
            throw new Error('No scenes found in EA-065');
        }
