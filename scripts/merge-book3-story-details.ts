...
/**
 * Merge detailed story information from Book_3_Sphere_of_Influence_Outline.json into l_outline.json
 * This script adds hero_journey_beat, character_arcs, story_gaps_addressed, 
 * location_details, and series_connections to all Book 3 chapters
 */
...
async function mergeBook3StoryDetails() {
  console.log('🚀 Starting merge of Book 3 story details');
  
  try {
    // Load the Book_3_Sphere_of_Influence_Outline.json file
    const book3OutlinePath = resolve(__dirname, '../lore/json/storyline/Book_3_Sphere_of_Influence_Outline.json');
    const book3Data: BookOutline = JSON.parse(readFileSync(book3OutlinePath, 'utf-8'));
    ...
    // Create a map of chapter data from Book_3_Sphere_of_Influence_Outline.json
    const chapterDataMap = new Map<number, Chapter>();
    
    book3Data.structure.forEach(section => {
    ...
    // Navigate to Book 3 in l_outline.json and add the missing fields
    ...
    // Find Book 3 in the structure
    let book3Found = false;
    ...
    for (const trilogyKey of Object.keys(trilogyBooks)) {
      ...
      if (trilogy.trilogy_books) {
        for (const [bookKey, bookData] of Object.entries(trilogy.trilogy_books as any)) {
          // Check if this is Book 3 (MT 3)
          if ((bookData as any).unique_identifier === 'MT 3') {
            book3Found = true;
            console.log(`✅ Found Book 3: ${(bookData as any).fiction_novel_title}`);
...
// Run the merge
mergeBook3StoryDetails().catch(console.error);