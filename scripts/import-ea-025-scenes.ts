...
async function main() {
  console.log('🔍 Searching for EA-025 in outline...\n');
  ...
  console.log(`✅ Found EA-025: ${ea025Data.title}`);
  console.log(`   Scenes: ${ea025Data.scenes?.length || 0}\n`);

  // Find the chapter - it should be chapter 25 in the same book as EA-023
  ...
  if (existingChapter.length > 0) {
    console.log('✅ EA-025 chapter already exists\n');
    chapter = existingChapter[0];
  } else {
    // Find chapter 25 in the same book
    const ch25 = await db
      .select()
      .from(chapters)
      .where(eq(chapters.chapterNumber, 25))
      .limit(10);

    const correctChapter = ch25.find((ch) => ch.bookId === bookId);

    if (!correctChapter) {
      console.error('❌ Chapter 25 not found in the same book as EA-023');
      process.exit(1);
    }

    chapter = correctChapter;
    console.log(`✅ Found Chapter 25: ${chapter.title}\n`);

    // Update with unique identifier and outline data
    await db
      .update(chapters)
      .set({
        uniqueIdentifier: 'EA-025',
        title: ea025Data.title,
      })
      .where(eq(chapters.id, chapter.id));

    console.log('✅ Updated chapter with EA-025 identifier\n');
  }
  ...
  for (const sceneData of ea025Data.scenes || []) {
    const sceneInsert = {
      chapterId: chapter.id,
      chapterUniqueIdentifier: 'EA-025',
  ...
  console.log('\n🎉 EA-025 basic import complete!');
  console.log('\nNext steps:');
  console.log('1. Complete chapter-level fields');
  console.log('2. Add all scene fields including save_the_cat_beat');
