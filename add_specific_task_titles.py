#!/usr/bin/env python3
"""
Script to add specific_task_group_title to personality profiles 34+.
...
def add_specific_task_titles_to_profiles(profile_data: Dict[str, Any], task_titles: Dict[str, str]) -> Dict[str, Any]:
    """Add specific_task_group_title to personality profiles 34+ in correct position."""
    updated_count = 0
    
    # Target profiles 34-39 (and beyond if they exist)
    for profile_num in range(34, 50):
        ...
        if profile_key in profile_data:
            ...
            if isinstance(profile, dict):
                ...
                if unique_id and unique_id in task_titles:
                    ...
                    # Check if specific_task_group_title is missing
                    if "specific_task_group_title" not in profile:
                        ...
                        print(f"{profile_key} already has specific_task_group_title: {profile.get('specific_task_group_title')}")
                else:
                    if unique_id:
                        print(f"No task title found for {profile_key} with unique_identifier: {unique_id}")
...
def main():
    ...
    # Show the specific titles we're looking for
    target_identifiers = ["STG 1.3.4.2", "STG 1.3.4.3", "STG 1.3.5.1", "STG 1.3.5.2", "MAT 1.3", "STG 1.3.6.1"]
    print("\nTarget identifiers and their titles:")
    ...
    updated_profile_data = add_specific_task_titles_to_profiles(profile_data, task_titles)
