...
function CharacterCard({ character }: { character: Character }) {
  return (
    <Link href={`/characters/${character.slug}`}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 flex flex-col items-center transition-all duration-300 hover:shadow-xl hover:-translate-y-1 border border-gray-200 dark:border-gray-700 cursor-pointer">
        ...
        <h2 className="text-lg font-bold text-gray-900 dark:text-white text-center group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors mb-2 line-clamp-2">
          {character.name}
        </h2>
        {character.role && (
          <div className="text-sm text-gray-500 dark:text-gray-400 text-center mb-2 font-medium">
            {character.role}
          </div>
        )}
